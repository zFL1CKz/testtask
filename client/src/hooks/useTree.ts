...
export default function useTree (division: IDivision | null): ITree {
  ...
  const subDivisions = divisions?.filter(div => div.parentDivisionId === division?.id && recursive(div)) ?? []

  function recursive(div: IDivision): IDivision[] {
    allNestedDivisions.push(div)
    const nestedDivisions = divisions?.filter(subDivision => subDivision.parentDivisionId === div?.id && recursive(subDivision)) ?? []
